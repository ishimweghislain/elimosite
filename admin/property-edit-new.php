        ...
        function uploadImagesAjax(files) {
            ...
            async function uploadFileIter(index) {
                ...
                while (!success && retries < maxRetries) {
                    try {
                        ...
                        retries++;
                        console.error(`Upload failed for file ${index}, retry ${retries}/${maxRetries}:`, error);
                ...
                if (!success) {
                    failedCount++;
                    console.error(`Failed to upload file ${index} after ${maxRetries} attempts.`);
                }
            ...
            function finishUpload() {
                ...
                if (uploadedCount > 0) {
                    ...
                    uploadMessage.textContent = `✓ Successfully uploaded ${uploadedCount} image(s)${failedCount > 0 ? `. ${failedCount} failed after multiple attempts.` : '!'}`;
                } else {
                    progressBar.classList.add('bg-danger');
                    uploadMessage.classList.add('text-danger');
                    uploadMessage.textContent = `✗ All uploads failed. Please check your internet connection and try again.`;
                }
